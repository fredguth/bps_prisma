...
FROM
  ...
  LEFT JOIN (
    ...
    FROM
      PJ
  ) AS PJFabricante ON ((item.cnpj_fabricante = PJFabricante.cnpj))
  LEFT JOIN (
    SELECT
      cnpj,
      razao_social
    FROM
      PJ
  ) AS PJFornecedor ON ((item.cnpj_fornecedor = PJFornecedor.cnpj))
  LEFT JOIN (
    SELECT
      id,
      "data"
    FROM
      compra
  ) AS compra ON ((item.compra_id = compra.id));